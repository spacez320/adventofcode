...
find_largest_sum_of_groups([Next|Tail], '', NextSum, Sum)
  when NextSum > Sum -> find_largest_sum_of_groups(Tail, Next, 0, NextSum);
...
find_largest_sum_of_groups([Next|Tail], NextNum, NextSum, Sum) ->
  io:format("Reading: ~p~n", [Next]), find_largest_sum_of_groups(Tail, Next, NextNum + NextSum, Sum);
...
find_largest_sum_of_groups([], _, _, Sum) -> Sum.
